(function () {
  ...
  progressFileInput.addEventListener("change", async () => {
    ...
    try {
      ...
      const blocks = buildContentBlocks(
        extracted,
        "В этом файле может быть НЕСКОЛЬКО отдельных актов выполненных работ (актов приёмки) от субподрядчика — возможно, на разных страницах, друг за другом, или даже на одной странице/фото. Внимательно проверь ВЕСЬ файл целиком (все страницы) и не объединяй разные акты в один: если у документов разные даты, разные суммы или это визуально отдельные бланки/подписи — это разные акты, и каждый должен быть отдельным элементом списка. Для каждого найденного акта извлеки: итоговую сумму выполненных работ, дату акта в формате YYYY-MM-DD, краткое описание/суть работ одной фразой, и список использованных материалов с их количеством числом, если они указаны. Если в файле только один акт — верни список из одного элемента."
      );
      const schema = {
        type: "object",
        properties: {
          acts: {
            type: "array",
            description: "Список ВСЕХ отдельных актов выполненных работ, найденных в файле — по одному элементу на каждый акт, даже если их много",
            items: {
              type: "object",
              properties: {
